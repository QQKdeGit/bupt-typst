...
这是一张表。注意到了吗？这里并没有自动段首空格，因为我们还在上一段里。要想在后面开启新的一段，需要自己手动输入换行符“\\”#footnote[换行符为单个反斜杠，但也可以输入\#linebreak来换行]。后面没有空格的原理是一样的。
...
这是一个参考文献的引用#super([[1]])，但目前还不支持 bibTex 文件来导入参考文献，只能通过自己写参考文献和引用的上标来实现。
...
#text(font: (FONTSET.at("English"), FONTSET.at("Song")), size: 10.5pt)[
  ...
  // 期刊中的文献
  [2] 作者.文献名称[J].期刊名称.卷号（期号）.年,月:页码范围.
  ...
  （未提及的参考资料类型，请参照知网GB/T 7714-2015格式引用）
]
